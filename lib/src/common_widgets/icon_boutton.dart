...
class CIconButton extends StatelessWidget {
  const CIconButton({super.key, required this.label, required this.onPressed, this.enabled = true, this.icon});
  final String label;
  ...
  Widget build(BuildContext context) {
    return ElevatedButton.icon(
        ...
        icon: Icon(icon, color: AppTheme.getTheme().primaryColor, size: 15,), 
        label: Text(label, 
          style: AppTheme.getTheme().textTheme.subtitle2!
                .copyWith(color: AppTheme.getTheme().primaryColor, 
