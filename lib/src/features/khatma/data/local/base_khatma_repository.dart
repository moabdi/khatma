...
abstract class BaseKhatmaRepository extends LocalKhatmaRepository {
  ...
  Future<Khatma?> getById(String id) async {
    ...
    if (jsonString != null) {
      ...
      if (khatmaData['status'] == KhatmaStatus.deleted.name) {
        return null;
      }

      return Khatma.fromJson(khatmaData);
    }
    return null;
  }
  ...
  Future<List<Khatma>> getKhatmasNeedingSync() async {
    ...
    return getBoxValues(box)
        .map((jsonString) {
          ...
          if (khatmaData['lastSync'] != null) return null;
  ...
  Future<List<CompletionHistory>> getHistoryNeedingSync() async {
    ...
    return getBoxValues(box)
        .map((jsonString) {
          ...
          if (historyData['lastSync'] != null) return null;

          return CompletionHistory.fromJson(historyData);
        })
